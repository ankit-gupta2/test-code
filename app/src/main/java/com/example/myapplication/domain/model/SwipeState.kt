...
open class SwipeState<T>(
    initialValue: T,
    internal val animationSpec: AnimationSpec<Float> = AnimationSpec,
    internal val confirmStateChange: (newValue: T) -> Boolean = { true }
) {

    class CustomDraggableState(val onDelta: (Float) -> Unit) : DraggableState {
    ...
    val draggableState = CustomDraggableState {
...
//    }
    ...
    companion object {
        ...
        ) = Saver<SwipeState<T>, T>(
            save = { it.currentValue },
            restore = { SwipeState(it, animationSpec, confirmStateChange) }
        )
    }
}
